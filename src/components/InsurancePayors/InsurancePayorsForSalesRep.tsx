import {
  getRevenueInsurancePayorsBySalesRepIdAPI,
  getVolumeInsurancePayorsBySalesRepIdAPI,
...
const InsurancePayorsForSalesRep = ({
  searchParams,
  pageName,
  tabValue,
  selectedCaseValue,
  ...
  const getVolumeDetailsOfInsrancePayors = async (queryParams: any) => {
    setLoading(true);
    try {
      let response = await getVolumeInsurancePayorsBySalesRepIdAPI({
        pageName,
        id: id as string,
        queryParams,
      });
      ...
      if (response?.status == 200 || response?.status == 201) {
        ...
        const result = [
          ...
          { value: completeCases, dolorSymbol: false },
          { value: totalPending, dolorSymbol: false },
        ];
  ...
  const getRevenueDetailsOfInsrancePayors = async (queryParams: any) => {
    setLoading(true);
    try {
      let response = await getRevenueInsurancePayorsBySalesRepIdAPI({
        pageName,
        id: id as string,
        queryParams,
      });
    ...
    <div style={{ position: "relative" }}>
      ...
      >
        <ExportButton
          ...
          disabled={
            insuranceData?.length === 0 || tabValue == "Revenue" ? true : false
          }
        />
      </div>
      <SingleColumnTable
...
export default InsurancePayorsForSalesRep;
